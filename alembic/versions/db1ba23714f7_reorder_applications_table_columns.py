"""reorder applications table columns

Revision ID: db1ba23714f7
Revises: a09508efc6df
Create Date: 2025-12-22 14:01:42.123456
...
revision: str = 'db1ba23714f7'
down_revision: Union[str, None] = 'a09508efc6df'
...
def upgrade() -> None:
    ...
    # Step 2: Copy all data from applications to applications_temp
    op.execute("""
        INSERT INTO applications_temp (
    ...
    # Step 7: Rename temp table to applications
    op.rename_table('applications_temp', 'applications')
...
def downgrade() -> None:
    ...
    Note: This will restore the "wrong" order from before.
    """

    # Step 1: Create temp table with old column order
    op.execute("""
        CREATE TABLE applications_temp (
            application_id BIGINT NOT NULL,
            job_id BIGINT NOT NULL,
    ...
    # Step 3-9: Same drop/rename/recreate process as upgrade
    op.drop_index('ix_applications_job_status', table_name='applications')
    op.drop_index('ix_applications_workflow_id', table_name='applications')
    op.drop_index('ix_applications_account_id', table_name='applications')
    op.drop_index('ix_applications_company_id', table_name='applications')
